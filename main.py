...
def producer() -> None:
    ...
    for _ in range(SIZE * 2): #we just produce twice the buffer size for testing
        ...
        empty.acquire() #DECREMENT the EMPTY semaphore, which indicates that there is one less empty slot

        with mutex:             #use lock, and insert an item into the buffer
            buffer.insert(item)

        full.release()  #once item is inserted, INCREMENT the full semaphore, indicating that there is one more full
